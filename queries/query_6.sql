-- ====================================================================
-- CONSULTA 6: Junção de 3+ Relações (Fãs do Estúdio MAPPA)
-- Objetivo: Comparar Múltiplos JOINS (Flat) vs Subconsultas Aninhadas (Deep).
--           Cenário: Listar usuários que deram nota 10 para animes da 'MAPPA'.
-- ====================================================================

USE `animes_db`;

DELIMITER //

-- ==================================================================
-- VERSÃO A: Múltiplos INNER JOINs (Abordagem Declarativa / Flat)
-- O otimizador escolhe a melhor ordem para cruzar as 4 tabelas.
-- ==================================================================
DROP PROCEDURE IF EXISTS TesteConsulta6_VersaoA //

CREATE PROCEDURE TesteConsulta6_VersaoA()
BEGIN
    DECLARE i INT DEFAULT 0;
    DECLARE start_time DATETIME(6);
    DECLARE end_time DATETIME(6);
    DECLARE exec_time DECIMAL(20,6);
    DECLARE media_final DECIMAL(20,6);

    DROP TEMPORARY TABLE IF EXISTS TempTimesA;
    CREATE TEMPORARY TABLE TempTimesA (
        Execucao VARCHAR(50),
        Tempo_Segundos DECIMAL(20,6)
    );
    
    WHILE i < 5 DO
        SET start_time = NOW(6);
        
        -- QUERY A (JOIN PLANO)
        SELECT DISTINCT U.Username
        FROM USUARIO U
        INNER JOIN WATCHLIST W ON U.UsuarioID = W.UsuarioID_FK
        INNER JOIN ANIME A ON W.AnimeID_FK = A.AnimeID
        INNER JOIN ESTUDIO E ON A.EstudioID_FK = E.EstudioID
        WHERE E.Nome = 'MAPPA' AND W.NotaDada = 10;
        
        SET end_time = NOW(6);
        
        SET exec_time = (TIMESTAMPDIFF(MICROSECOND, start_time, end_time) / 1000000);
        INSERT INTO TempTimesA VALUES (CONCAT('Tentativa ', i+1), exec_time);
        
        SET i = i + 1;
    END WHILE;

    SELECT AVG(Tempo_Segundos) INTO media_final FROM TempTimesA;

    SELECT Execucao, Tempo_Segundos FROM TempTimesA
    UNION ALL
    SELECT '--- MÉDIA FINAL (Versão A) ---', media_final;
    
    DROP TEMPORARY TABLE TempTimesA;
END //


-- ==================================================================
-- VERSÃO B: Subconsultas Profundamente Aninhadas (IN)
-- Força uma lógica de "dentro para fora" (Estúdio -> Anime -> Watchlist -> Usuário).
-- ==================================================================
DROP PROCEDURE IF EXISTS TesteConsulta6_VersaoB //

CREATE PROCEDURE TesteConsulta6_VersaoB()
BEGIN
    DECLARE i INT DEFAULT 0;
    DECLARE start_time DATETIME(6);
    DECLARE end_time DATETIME(6);
    DECLARE exec_time DECIMAL(20,6);
    DECLARE media_final DECIMAL(20,6);

    DROP TEMPORARY TABLE IF EXISTS TempTimesB;
    CREATE TEMPORARY TABLE TempTimesB (
        Execucao VARCHAR(50),
        Tempo_Segundos DECIMAL(20,6)
    );
    
    WHILE i < 5 DO
        SET start_time = NOW(6);
        
        -- QUERY B (Aninhamento Profundo com IN)
        SELECT Username
        FROM USUARIO
        WHERE UsuarioID IN (
            SELECT UsuarioID_FK
            FROM WATCHLIST
            WHERE NotaDada = 10 AND AnimeID_FK IN (
                SELECT AnimeID
                FROM ANIME
                WHERE EstudioID_FK IN (
                    SELECT EstudioID
                    FROM ESTUDIO
                    WHERE Nome = 'MAPPA'
                )
            )
        );
        
        SET end_time = NOW(6);
        
        SET exec_time = (TIMESTAMPDIFF(MICROSECOND, start_time, end_time) / 1000000);
        INSERT INTO TempTimesB VALUES (CONCAT('Tentativa ', i+1), exec_time);
        
        SET i = i + 1;
    END WHILE;

    SELECT AVG(Tempo_Segundos) INTO media_final FROM TempTimesB;

    SELECT Execucao, Tempo_Segundos FROM TempTimesB
    UNION ALL
    SELECT '--- MÉDIA FINAL (Versão B) ---', media_final;
    
    DROP TEMPORARY TABLE TempTimesB;
END //

DELIMITER ;


-- =================================================================
-- EXECUÇÃO DOS TESTES
-- =================================================================

CALL TesteConsulta6_VersaoA();
CALL TesteConsulta6_VersaoB();